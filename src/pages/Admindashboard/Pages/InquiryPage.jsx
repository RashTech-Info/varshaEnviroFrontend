...
const AdminInquiry = () => {
  ...
  const fetchInquiries = async () => {
    try {
      const res = await axios.get(`${API_URL}/getInquiry`, {
        withCredentials: true,
      });
      console.log("response of inquiry--", res);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setInquiries(
        inquiries.filter((inquiry) => inquiry._id !== inquiryToDelete)
      );
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await axios.patch(
        `${API_URL}/updateInquiryStatus/${id}`,
        { status: newStatus },
        { withCredentials: true }
      );
  ...
  const filteredInquiries = inquiries.filter((inquiry) => {
    const matchesSearch =
      ...
      (inquiry.eventType || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) || // Assuming eventType might be part of search
      (inquiry.description || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      (inquiry.companyName || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) || // Added companyName to search
      (inquiry.product || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) || // Added product to search
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={index}
              className="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-gray-100 p-3 sm:p-4 shadow-sm"
            >
              <div className="flex items-center">
                <div
                  className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${stat.color} mr-2 sm:mr-3`}
                ></div>
                <div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
                    <div className="flex-1 mb-3 sm:mb-0">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-3 sm:gap-0">
                        ...
                        <div
                          className={`flex items-center px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium border ${statusConfig.color}`}
                        >
                          <div
                            className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full mr-1 sm:mr-2 ${statusConfig.dot}`}
                          ></div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-3 sm:mb-4">
                        <div className="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg sm:rounded-xl">
                          <Building
                            size={16}
                            className="text-[#016630] mr-2 sm:mr-3"
                          />
                          <div>
                      ...
                      <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-3 sm:mb-4">
                        <h4 className="font-semibold text-gray-900 mb-1 sm:mb-2 flex items-center text-sm sm:text-base">
                          <FileText
                            size={14}
                            className="mr-1 sm:mr-2 text-[#016630]"
                          />
                          Description
                        </h4>
