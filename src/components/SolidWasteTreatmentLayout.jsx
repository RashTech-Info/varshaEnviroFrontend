...
import { Leaf, Scissors, ChevronRight } from 'lucide-react';
import { StickyQuoteButton } from '../components/sticky-quote-button'; // Import your popup inquiry form

const solidWasteProducts = [
  {
    id: 'organic-waste-composter',
    name: 'Organic Waste Composter',
    path: '/services/organic-waste-composter',
    icon: <Leaf className="w-6 h-6" />,
  },
  {
    id: 'garden-waste-shredder',
    name: 'Garden Waste Shredder',
    path: '/services/garden-waste-shredder',
    icon: <Scissors className="w-6 h-6" />,
  },
];

export default function SolidWasteTreatmentLayout({ children }) {
  const location = useLocation();
  const [openForm, setOpenForm] = useState(false);

  const matchedProduct = solidWasteProducts.find((p) => p.path === location.pathname);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Sustainable solid waste treatment solutions for environmental management
          </p>
        </div>
          ...
          <div className="lg:w-1/3 xl:w-1/4">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-8">
              ...
              <div className="space-y-2">
                {solidWasteProducts.map((product) => (
                  <Link
                    key={product.id}
                    to={product.path}
              ...
              <Link
                to="/services/solid-waste-treatment"
